...
const register = (registerObj) => {
    ...
    //Pass length Validation
      if(registerObj.password.length < 8) { return resolve("Password not long enough") }
    ...
    try {
      newUser.save( (err) => {
        ...
        console.log(err);
        if(err) { return errorBL.registerHandle(err)}
      }).then((user) => {
      ...
      return resolve({error: errors, status: 400})}
     })
